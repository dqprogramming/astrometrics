...
from journals.models import Journal, Language, PackageBand, Publisher, Subject


class JournalSearchView(ListView):
    """Advanced journal search view with filtering.
    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = self._apply_search(
                queryset, search_query, search_fields
            )
        else:
            # Apply ordering only when not searching
            # (search has its own ranking)
    ...
    def _apply_search(self, queryset, search_query, search_fields):
        ...
        if "subjects" in search_fields:
            search_vectors.append(
                SearchVector("subjects__name", weight="D", config="english")
            )

        if "issn" in search_fields:
            search_vectors.append(
                SearchVector("issn", weight="A", config="english")
            )

        if "journal_owner" in search_fields:
            search_vectors.append(
                SearchVector("journal_owner", weight="C", config="english")
            )
    ...
    def _apply_filters(self, queryset):
        ...
        if publisher_ids:
            ...
            publisher_ids = [
                int(pid) for pid in publisher_ids if pid.isdigit()
            ]
        ...
        if package_band_ids:
            package_band_ids = [
                int(pid) for pid in package_band_ids if pid.isdigit()
            ]
            ...
            try:
                if articles_min:
                    queryset = queryset.filter(
                        normalized_articles__gte=Decimal(articles_min)
                    )
                if articles_max:
                    queryset = queryset.filter(
                        normalized_articles__lte=Decimal(articles_max)
                    )
            except (ValueError, TypeError):
                pass
        ...
        if impact_min or impact_max:
            try:
                if impact_min:
                    queryset = queryset.filter(
                        wos_impact_factor__gte=Decimal(impact_min)
                    )
                if impact_max:
                    queryset = queryset.filter(
                        wos_impact_factor__lte=Decimal(impact_max)
                    )
            except (ValueError, TypeError):
                pass
        ...
        if archive_years_min:
            try:
                queryset = queryset.filter(
                    archive_years__gte=int(archive_years_min)
                )
            except (ValueError, TypeError):
                pass
...
def journal_stats_view(request):
    ...
    total_cost = (
        Journal.objects.aggregate(Sum("cost_gbp"))["cost_gbp__sum"] or 0
    )
    avg_cost = Journal.objects.aggregate(Avg("cost_gbp"))["cost_gbp__avg"] or 0
    avg_articles = (
        Journal.objects.aggregate(Avg("normalized_articles"))[
            "normalized_articles__avg"
        ]
        or 0
    )
    ...
    impact_factor_count = Journal.objects.filter(
        wos_impact_factor__isnull=False
    ).count()

    # License distribution
    license_distribution = (
        Journal.objects.values("licensing")
    ...
    context = {
        ...
        "doaj_percentage": (doaj_count / total_journals * 100)
        if total_journals
        else 0,
        "scopus_count": scopus_count,
        "scopus_percentage": (scopus_count / total_journals * 100)
        if total_journals
        else 0,

...
from journals.models import Journal, Publisher, Subject, Language


class PublicJournalSearchView(ListView):
    """
    Simplified public-facing journal search view.
    Less complex than the admin version, focusing on basic search and filtering.
    """
    model = Journal
    ...
    def _apply_search(self, queryset, search_query):
        """
        Apply PostgreSQL full-text search across main fields including subjects.
        """
    ...
    def _apply_filters(self, queryset):
        ...
        if subject_id:
            try:
                queryset = queryset.filter(subjects__id=int(subject_id)).distinct()
        ...
        if language_id:
            try:
                queryset = queryset.filter(languages__id=int(language_id)).distinct()
            except (ValueError, TypeError):
                pass
...
def public_journal_detail_view(request, pk):
    ...
    ).exclude(pk=journal.pk).distinct()[:6]

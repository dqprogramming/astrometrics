...
class Subject(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        unique=True,
        help_text="Subject or discipline name"
    )
...
class PackageBand(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text="Descriptive name of the package band"
    )
...
class Journal(models.Model):
    ...
    journal_url = models.URLField(
        blank=True,
        help_text="Link to journal website"
    )
    publisher_url = models.URLField(
        blank=True,
        help_text="Link to publisher website"
    )
    ...
    description = models.TextField(
        blank=True,
        help_text="Journal description and scope"
    )
    ...
    in_doaj = models.BooleanField(
        default=False,
        help_text="Whether journal is listed in Directory of Open Access Journals"
    )

    # Indexing
    in_scopus = models.BooleanField(
        default=False,
        help_text="Whether journal is indexed in Scopus"
    )
    ...
    archive_available_diamond_oa = models.TextField(
        blank=True,
        help_text="Details about diamond OA archive availability"
    )
    ...
    def cost_per_article(self):
        ...
        if self.cost_gbp and self.normalized_articles and self.normalized_articles > 0:
...
class ImportLog(models.Model):
    ...
    error_log = models.TextField(
        blank=True,
        help_text="Detailed error messages and warnings"
    )

...
from .models import Journal, Publisher, Subject, Language, ImportLog
...
class JournalAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        qs = super().get_queryset(request)
        return qs.select_related('publisher').prefetch_related('languages', 'subjects')
    ...
    def mark_not_in_doaj(self, request, queryset):
        ...
        self.message_user(request, f'{count} journal(s) marked as not in DOAJ.')
    mark_not_in_doaj.short_description = 'Mark selected journals as not in DOAJ'
    ...
    def export_to_csv(self, request, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename="journals_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        
        writer = csv.writer(response)
        writer.writerow([
            'Title', 'Publisher', 'Package Band', 'Cost (GBP)', 
            'Normalized Articles', 'Cost per Article', 'ISSN',
            'In DOAJ', 'In Scopus', 'WOS Impact Factor',
            'Languages', 'Subjects', 'License'
        ])
        
        for journal in queryset:
            languages = ', '.join(lang.name for lang in journal.languages.all())

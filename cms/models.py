...
class ManifestoPageSettings(models.Model):
    """Singleton model for the Our Manifesto page content.

    Only one row should exist — use ManifestoPageSettings.load() to
    fetch-or-create it.
    """
    ...
    text_body = models.TextField(
        blank=True,
        help_text="Main text block — rich text (bold, italic, underline, sub, sup only)",
    )

    # Organise section (upper blue)
    organise_heading = models.CharField(
        max_length=500,
        default="To do this, we must organise.",
        help_text="Organise section heading",
    )
    organise_body = models.TextField(
        blank=True,
        help_text="Organise section body — rich text",
    )

    # Achievable section (lower blue)
    achievable_heading = models.CharField(
        max_length=500,
        default="Our task is ambitious, yet achievable.",
        help_text="Achievable section heading",
    )
    achievable_body = models.TextField(
        blank=True,
        help_text="Achievable section body — rich text",
    )

    # CTA button (blue section)
    cta_button_text = models.CharField(
        max_length=100,
        default="It starts here",
    ...
    # Final section
    final_heading = models.CharField(
        max_length=500,
        default="Free access to hundreds of the world's leading academic journals.",
        help_text="Final section heading",
    )
    final_image_alt = models.CharField(
        max_length=255,
        default="A modern library representing free access to academic journals",
        help_text="Alt text for the final section image",
    )
    final_button_text = models.CharField(
        max_length=100,
        default="Speak to us",
        help_text="Final section button text",
    )
    final_button_url = models.CharField(
        max_length=500,
        default="/our-team/",
        help_text="Final section button URL",
    )
    ...
    class Meta:
        verbose_name = _("Manifesto Page Settings")
        verbose_name_plural = _("Manifesto Page Settings")

    def __str__(self):
        return "Manifesto Page Settings"

    CACHE_KEY = "manifesto_page_settings"
    CACHE_TTL = 60 * 60  # 1 hour
    ...
    def save(self, *args, **kwargs):
        self.pk = 1
        self.text_body = sanitize_html(self.text_body)
